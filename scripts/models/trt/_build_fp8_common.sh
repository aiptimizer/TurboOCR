#!/usr/bin/env bash
# Shared helpers for build_<model>_fp8.sh wrappers. Source this file from
# the per-model script and then call `run_fp8_build`.
#
# Required env from caller:
#   MODEL_TYPE         — TRT type label (det / layout / table_struct_nemotron / formula_encoder)
#   ENV_FLAG_NAME      — TURBO_OCR_<MODEL>_FP8
#   ONNX_PATH          — absolute path to source FP ONNX
#   CACHE_GLOB         — glob (in CACHE_DIR) of stale engine files to prune
#
# Mirrors scripts/models/trt/_build_int8_common.sh almost verbatim — different env-var
# names, different QDQ suffix (_fp8.onnx vs .int8.onnx), different quantizer
# script. The build-via-server-boot pattern, FP16-baseline snapshotting, and
# ≥5 % latency gate are intentionally identical.
...
run_fp8_build() {
  ...
  if [[ -f "${qdq_path}" ]] && [[ "${qdq_path}" -nt "${ONNX_PATH}" ]]; then
    ...
    log "running modelopt FP8 PTQ via quantize_onnx_fp8.py"
    "${VENV_PY}" "${REPO_ROOT}/scripts/models/trt/quantize_onnx_fp8.py" \
  ...
  local log_file
  log_file="$(mktemp -t "${MODEL_TYPE}_fp8_build.XXXXXX.log")"
  ...
  if (( ${#candidates[@]} > 0 )); then
    fp8_engine="$(ls -1t "${candidates[@]}" 2>/dev/null | head -n1)"
  fi
  shopt -u nullglob
  [[ -n "${fp8_engine}" ]] || die "FP8 engine not found in ${CACHE_DIR} after build"
  if [[ "${fp8_engine}" == "${fp16_engine}" ]]; then
    die "FP8 build did not produce a new cache entry (cache key didn't change?)"
  fi
  log "FP8 engine   : ${fp8_engine}"

  log "running latency gate (3 warmup + 20 timed images)"
  if ! "${VENV_PY}" "${REPO_ROOT}/scripts/models/trt/fp8_latency_gate.py" \
        --fp16-engine "${fp16_snap}" \
        --fp8-engine "${fp8_engine}" \
        --model-type "${MODEL_TYPE}"; then
    log "==> LATENCY GATE FAILED for ${MODEL_TYPE}"
    log "    This is the 'FP8 storage with FP16 compute' failure mode (see"
    log "    /tmp/fp8_research.md §Q6 — TRT 10.15.1 release-note advisory on"
    log "    group convs). Removing the FP8 engine so the next server boot"
    log "    reverts to FP16."
    rm -f "${fp8_engine}"
    rm -f "${fp16_snap}"
    die "drop --model-type=${MODEL_TYPE} from the FP8 experiment"
  fi

  rm -f "${fp16_snap}"
  log "==> PASS — FP8 engine for ${MODEL_TYPE} is faster than FP16 and retained."
  log "    Engine: ${fp8_engine}"
  log "    Build log: ${log_file}"
}
