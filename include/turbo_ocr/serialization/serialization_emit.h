...
inline void assign_layout_ids(std::vector<OCRResultItem> &results,
  ...
  //
  //    id == index is load-bearing: LayoutBox::parent_id is a positional
  //    reference recorded by postfilter_layout_boxes, so it only resolves to
  //    the right region while ids stay equal to positions. Renumbering ids
  //    here to anything else would silently break the hierarchy.
