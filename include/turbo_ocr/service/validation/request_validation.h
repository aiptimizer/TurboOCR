...
// The default routing policy for image endpoints. SUPPORTED, unconditionally.
//
// This used to be `#ifdef USE_CPU_ONLY ? kUnsupported : kSupported` — a CMake
// option deciding a REQUEST-VALIDATION rule, inside the layer whose whole point
// is being transport- and build-free. That was answerable when the CPU pipeline
// genuinely had no routing plumbing and lived in a separate binary. It is not
// now: there is one server, it always runs UnifiedOcrPipeline, and that threads
// `opts.routing_override` into run_with_layout / run_encoded on every backend
// (src/pipeline/unified/make_infer_func.cpp). The macOS/Apple server is built
// with USE_CPU_ONLY=ON, so the old branch rejected ?route_table= on a Metal
// backend that would have honoured it — and told the operator it was because of
// "the CPU build" they never chose.
...
inline constexpr const char *kRoutingUnsupportedCpu =
    "per-request routing overrides are not supported on this endpoint (the "
...
struct EndpointSpec {
  ...
  // Which optional capabilities this endpoint can actually ACT on. The shared
  // parser only parses/gates capabilities in this mask; the rest fall through
  // to validate_params' classification exactly like any other unsupported
  // param (lenient: ignored + X-Ignored-Params; strict: 400).
  //
  // WHY: when the parser briefly parsed EVERY capability everywhere,
  // ?autorotate=1 on the image endpoints — which never rotate — flipped from
  // v3.5.0's tolerated-and-ignored to a hard 400 whenever the doc-orientation
  // model wasn't loaded, and to a silent no-op when it was. An endpoint must
  // not accept (or reject on the availability of) a capability it cannot run.
  capability::CapabilityMask acts_on = default_acts_on();
  // The image-endpoint default: everything except DocOrientation, which only
  // the PDF paths apply (they rotate the page before OCR).
  [[nodiscard]] static constexpr capability::CapabilityMask default_acts_on() {
    return capability::CapabilityMask::all().set(
  ...
  // output / min_confidence / fields / editable / movable / mark_regions —
  // ONLY /ocr/pdf (parse_pdf_request); /ocr/stream shares pdf_options but
  // never parses these, so it must not accept them.
  bool pdf_doc_params = false;
  // embed (/ocr/markdown)
...
// Document-output params only /ocr/pdf honours (searchable/editable PDF,
// form-field proposals). Split from kPdfParams because /ocr/stream shares
// pdf_options but never parses these — leaving them in the shared group made
// the stream endpoint accept ?output=pdf and silently return NDJSON anyway.
// editable/movable/mark_regions were parsed by parse_pdf_request but missing
// from every group, so strict mode 400'd fully supported requests.
inline constexpr std::array<std::string_view, 6> kPdfDocParams = {
    "output", "min_confidence", "fields", "editable", "movable",
    "mark_regions"};
...
[[nodiscard]] constexpr bool is_known_param(std::string_view name) {
  ...
  return in_group(kOcrOptionParams, name) || in_group(kRoutingParams, name) ||
         ...
         in_group(kPdfDocParams, name) || in_group(kMarkdownParams, name);
}
...
[[nodiscard]] constexpr bool spec_allows(const EndpointSpec &spec,
  ...
  // `autorotate` is a capability param (DocOrientation) whose group home is
  // kPdfParams for historical reasons. An endpoint whose acts_on includes the
  // capability parses and honours it (image routes rotate at the seam since
  // make_infer_func::maybe_autorotate), so it must not simultaneously be
  // classified as an ignored/unknown param — that put "autorotate" in
  // X-Ignored-Params on a request that WAS rotated, and 400'd it under
  // strict params.
  if (name == "autorotate" &&
      spec.acts_on.get(capability::CapabilityId::DocOrientation))
    return true;
  if (spec.pixel_dims && in_group(kPixelDimParams, name)) return true;
  if (spec.pdf_options && in_group(kPdfParams, name)) return true;
  if (spec.pdf_doc_params && in_group(kPdfDocParams, name)) return true;
