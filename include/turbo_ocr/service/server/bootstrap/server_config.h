...
#include "turbo_ocr/analysis/classification/cls_options.h"
...
#include "turbo_ocr/service/server/language_paths.h"
...
struct ServerConfig {
  ...
  // Device backend (cpu|apple|nvidia|amd|intel); empty = auto-detect among the
  // backends compiled into this binary.
  std::string backend;           // TURBO_BACKEND / OCR_BACKEND / --backend
  // Which path to the silicon: "native"/"ultra" (the vendor graph engine),
  // "onnx"/"fast" (the .onnx on the vendor's ORT provider, no graph build), or
  // "" / "auto" = native when its artefact exists, else onnx. See
  // include/turbo_ocr/backend/engine_mode.h.
  std::string engine_mode;       // TURBO_ENGINE_MODE / --engine-mode
  ...
  // Render DPI when a request does not pass ?dpi= / OCRPDFRequest.dpi.
  // Was a bare literal 100 in THREE places (the gRPC service ctor and two
  // route registrations) with no way to change it — while every neighbouring
  // limit was already tunable. DPI is the primary quality/speed lever for PDF
  // OCR, so an operator tuning a deployment needs it more than most.
  int pdf_default_dpi    = 100;          // PDF_DEFAULT_DPI / --pdf-default-dpi
  // WorkPool queue depth (the HTTP admission bound). 0 = derive from the
  // pipeline pool, which is the historical behaviour: max(pool_size*32, 128).
  int work_queue_depth   = 0;            // WORK_QUEUE_DEPTH / --work-queue-depth
