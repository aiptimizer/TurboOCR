...
#include "turbo_ocr/core/model_catalog.h"
...
[[nodiscard]] inline ResolvedModel resolve_model(const char* rec_env, const char* det_env,
  ...
  if (const std::string model = env::env_or("OCR_MODEL", ""); !model.empty()) {
    if (const ModelEntry* e = find_model(model.c_str()))
      entry = e;
    else if (errors)
      errors->push_back("OCR_MODEL=\"" + model +
                        "\" is not a known model (valid: " + detail::model_names_csv() + ")");
  } else if (const std::string lang = env::env_or("OCR_LANG", ""); !lang.empty()) {
    entry = &detail::lang_alias_entry(lang.c_str());
    if (warnings)
      warnings->push_back("OCR_LANG is deprecated; use OCR_MODEL. OCR_LANG=\"" + lang +
                          "\" resolved to model \"" + std::string(entry->name) + "\"");
  }
