...
public:
  ...
  // token tensor [B, L] (the fused PP-FormulaNet-S graph, batched). ORT
  // allocates the [B,L] output on the host; tokens is filled row-major, L set
  // to the length, and `rows` to the batch rows the graph ACTUALLY returned —
  // which may be < B, and the caller must iterate `rows`, not B: `tokens` is
  // sized rows*L, so trusting the requested B reads past its end.
