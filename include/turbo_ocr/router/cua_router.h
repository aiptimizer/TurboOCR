...
// CPU-only routing stage. Pure on `classify`; the dispatch entrypoints
// are stubs (TablePipeline / FormulaPipeline implementations land in
// later tasks). One instance per pipeline-pool slot so the internal
// thread-local-style scratch buffers never get cross-thread access.
class CuaRouter {
 public:
  CuaRouter() noexcept;
  ~CuaRouter() noexcept;
