...
// UTC breakdown, one spelling per platform. POSIX gmtime_r and MSVC gmtime_s
// take their arguments in the OPPOSITE order and MSVC's returns errno_t, so
// this cannot be a macro alias — both call sites below go through it. Plain
// std::gmtime is not an option: it returns a pointer to a shared static, which
// two logging threads would race on.
inline void turbo_gmtime(const std::time_t &secs, std::tm &out) {
#if defined(_WIN32)
  gmtime_s(&out, &secs);   // (tm*, time_t*) — reversed vs POSIX
#else
  gmtime_r(&secs, &out);   // (time_t*, tm*)
#endif
}
