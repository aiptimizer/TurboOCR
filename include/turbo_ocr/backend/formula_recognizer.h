...
// IFormulaRecognizer — de-CUDA-typed formula recognition.
//
// Rebuild of src/backends/nvidia/stages/formula_recognizer.h. Two changes make it
// backend-agnostic (today it is typed on GpuImage + cudaStream_t, so the CPU
// sibling cannot implement it and defines its own cv::Mat API instead):
//   1. GpuImage      -> backend::ImageView
//   2. cudaStream_t  -> backend::DeviceQueue&
// The FormulaEngineResult value type, the async decouple, the deferred parser,
// and the CJK context hint are all preserved verbatim.
...
// Backend-agnostic formula recognition interface. Implementations may use any
// execution shape internally — an in-process graph driven by a host AR loop
// (PP-FormulaNet-S on ORT-CUDA / MPSGraph / MIGraphX / OpenVINO / ORT-CPU), or a
// remote VLM endpoint.
//
// Contract: load the on-disk artefact bundle from a directory, optionally load a
// tokenizer, then answer run() with a FormulaEngineResult per input box in input
// order.
...
public:
  ...
  // Tokenizer is backend-defined. Some backends embed their tokenizer in the
  // model bundle; pass an empty string in that case and the call is a no-op that
  // returns true.
  ...
  // Remote (HTTP/VLM) backends can split run() into a fast non-blocking submit
  // (D2H + PNG-encode + global-pool submit, done on the device pipeline worker)
  // and a later await+parse (done OFF the worker, on the work pool / HTTP
  // thread). Local backends leave these defaulted and keep the synchronous run()
  // — supports_async() stays false for them.
  [[nodiscard]] virtual bool supports_async() const noexcept { return false; }

  // Submit each box's crop and return one raw-response future per box, in input
  // order, WITHOUT blocking on the network. Default: not supported.
  ...
  // Turn one raw endpoint response (resolved from a submit_async future) into the
  // final LaTeX. Default identity; remote backends apply their parser.
  [[nodiscard]] virtual std::string
  parse_async_result(const std::string &raw) const { return raw; }

  // Return a SELF-CONTAINED parser for the deferred (async) path. Obtained on the
  // device worker while this recognizer is alive; the returned callable must
  ...
  // Default: identity. Async backends override to snapshot their parser.
  ...
  // pipeline skips its O(page-text) CJK scan for every backend that returns false
  // (all single-model engines), so the hint is computed only when it can change
  // routing. Only the auto composite overrides this.
  [[nodiscard]] virtual bool wants_context_hint() const noexcept { return false; }
};

// NOTE deliberately NO make_formula_recognizer(std::string_view) here — see
// the matching note in table_recognizer.h.
