...
// "image" class id, referenced by the oversized-image filter in
// layout_postfilter.h. Pinned to its label slot by the static_assert below.
inline constexpr int kImageClassId = 14; // "image"

// Classes the layout model intentionally emits *nested* inside a larger
// region — they are children, not duplicate subsets (paragraph_title in
// content, figure_title in image, footnote in footer, formula_number in
...
struct LayoutBox {
  ...
  int id = -1;
  // Containment parent: the `id` of the SMALLEST surviving region that
  // contains this one, or -1 for a top-level region. Assigned by
  // postfilter_layout_boxes (layout_postfilter.h) from the same >=90%-area
  // `layout_box_inside` predicate the LAYOUT_MERGE_MODE drop rule uses, so
  // the two can never disagree. The serializer omits the field when < 0, so
  // flat pages stay byte-identical to the pre-hierarchy response.
  //
  // INVARIANT: this is a POSITIONAL reference — the index of the parent in
  // the same layout vector. It equals the parent's `id` only because
  // assign_layout_ids (serialization_emit.h) assigns id = index. Anything
  // that reorders or erases entries in that vector must renumber parent_id
  // too, exactly as it would have to renumber `id` and OCRResultItem's
  // `layout_id`.
  int parent_id = -1;
  // Read-order index emitted natively by PP-DocLayoutV3 (column 6 of each
  // detection row); -1 when not provided. Kept straight from the model and
  // NOT serialised — reading order is still derived geometrically by
  // reading_order.cpp (class-bucketed XY-cut), so this only preserves the
  // model's own ordering signal for any future consumer.
  int read_order = -1;
