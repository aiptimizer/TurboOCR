...
Run with:
    pytest tests/integration/service/test_endpoints.py -v
"""
...
class TestOcrBatchEndpoint:
    ...
    def test_batch_degraded_slot_contract(self, server_url, test_png_base64):
        """A batch slot may come back DEGRADED — the published signal that a 200
        is not a clean result. Two halves are asserted here.

        1. The flag/warning pairing clients branch on: `*_degraded` is additive
           (absent unless set) and never appears without a human-readable
           `*_warning` beside it.
        2. A page that genuinely had no text is NOT flagged. `text_degraded`
           exists to tell "detection found regions, recognition returned
           nothing" apart from "the page was blank" — both serialize as
           `{"results": []}`, so a blank slot claiming degradation would make the
           signal meaningless. This is the side a client can force from outside
           the server; the firing side needs a recognizer that returns nothing
           and is pinned in the C++ suite (tests/cpp/pipeline/test_pipeline_detail.cpp).
        """
        ...
        r = requests.post(
            f"{server_url}/ocr/batch",
            json={"images": [blank_b64, test_png_base64]},
            timeout=20,
        )
        assert r.status_code == 200, r.text[:300]
        data = r.json()
        entries = data["batch_results"]
        assert len(entries) == 2
        # Both slots are valid PNGs: degradation is a signal on a slot the
        # server ran, never a per-slot decode error.
        assert data.get("errors", [None, None]) == [None, None]

        for i, entry in enumerate(entries):
            for stage in ("text", "table", "formula"):
                flag = entry.get(f"{stage}_degraded")
                if flag is None:
                    continue  # not set -> key omitted, which is the contract
                assert flag is True, (
                    f"slot {i}: {stage}_degraded is emitted only when true, "
                    f"got {flag!r}"
                )
                warning = entry.get(f"{stage}_warning")
                assert isinstance(warning, str) and warning, (
                    f"slot {i}: {stage}_degraded with no {stage}_warning to "
                    f"explain it (got {warning!r})"
                )

        assert entries[0].get("text_degraded") is None, (
            "a blank page has no detections and must not be flagged degraded: "
            f"{entries[0].get('text_warning')!r}"
        )
