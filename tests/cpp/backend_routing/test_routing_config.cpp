...
TEST_CASE("kind:local accepts every valid modality/engine pair",
  ...
  // "ppformulanet_plus_s" is the accurate alias for the shipped fast bundle
  // (its weights ARE PP-FormulaNet_plus-S); it must normalize to the
  // historical token at the parse seam so every backend's engine comparison
  // keeps a single string.
  turbo_ocr::backend_routing::BackendSpec alias;
  CHECK_NOTHROW(alias = parse_inline_backend(
      "formula", R"({"kind":"local","engine":"ppformulanet_plus_s"})"));
  CHECK(alias.engine == "ppformulanet_s");
}
...
TEST_CASE("synth: FORMULA_BACKEND=ppformulanet_plus_s normalizes to the "
          "historical token",
          "[routing_config]") {
  reset_env();
  ::setenv("FORMULA_BACKEND", "ppformulanet_plus_s", 1);
  auto t = load_routing_config();
  const auto &fb = t.backends.at(t.routes.at("formula"));
  CHECK(fb.kind == Kind::Local);
  // The shipped bundle IS PP-FormulaNet_plus-S; the alias must resolve to the
  // one token every backend compares against, not fork a second engine name.
  CHECK(fb.engine == "ppformulanet_s");
  ::unsetenv("FORMULA_BACKEND");
}
