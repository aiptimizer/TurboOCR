...
TEST_CASE("from_env accepts both model env spellings on every profile",
          "[server_config]") {
  // Both DET_ONNX and DET_MODEL are read on EVERY build; *_ONNX is canonical and
  // wins if both are set. The name used to fork on the profile, so a config set
  // with DET_ONNX was silently ignored on the CPU/host build — the portability
  // bug this contract fixes.
  {
    reset_env();
    ::setenv("DET_ONNX",  "trt_det.onnx", 1);
    ::setenv("DET_MODEL", "cpu_det.onnx", 1);
    // Canonical wins on BOTH profiles now (was: Cpu picked DET_MODEL).
    CHECK(ServerConfig::from_env(Profile::Gpu).det_onnx == "trt_det.onnx");
    CHECK(ServerConfig::from_env(Profile::Cpu).det_onnx == "trt_det.onnx");
  }
  {
    // The alias alone is honoured on every profile — this is the actual fix.
    reset_env();
    ::setenv("DET_MODEL", "only_model.onnx", 1);
    CHECK(ServerConfig::from_env(Profile::Gpu).det_onnx == "only_model.onnx");
    CHECK(ServerConfig::from_env(Profile::Cpu).det_onnx == "only_model.onnx");
  }
}
