...
// The OTSL->HTML converter itself. openai_policy::otsl_or_html adds the
// trim-detect / raw-sanitize passthrough on top of it and is covered separately
// in test_openai_policy.cpp; extract_latex, the other openai-endpoint parser, is
// in test_latex_extract.cpp.
//
// This includes the DEVICE-FREE declaring header, not turbo_ocr/analysis/table/vlm/
// vlm_table.h — that one also declares the CUDA-typed VLMTableRecognizer and
// therefore <cuda_runtime.h>, which is the only reason this suite used to be
// confined to the CUDA-only configure while the converter it tests runs in the
// CPU unified server.
