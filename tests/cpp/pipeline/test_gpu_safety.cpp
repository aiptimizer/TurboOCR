...
// (The two C4 get_with_timeout cases that stood here are GONE, with the
// primitive itself: the per-request future timeout belonged to
// pipeline_dispatcher.h, deleted with the CUDA orchestration. Its job — bounding
// how long a request waits for a busy device — is now UnifiedPipelinePool's
// acquire timeout, which is device-neutral and therefore covered in
// tests/cpp/pipeline/test_pipeline_pool.cpp on BOTH configures rather than only
// under a CUDA build. This file kept building against the dead header, so the
// whole GPU test binary failed to compile and nothing in it ran.)
