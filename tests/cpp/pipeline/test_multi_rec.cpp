...
// Local stand-in for what the script classifier hands the dispatcher. It used
// to be lang_cls::ScriptIdResult, deleted with the CUDA-only ScriptIdEngine;
// these cases never needed the engine, only the (script, confidence) pair the
// clamp below reads, so they keep testing the clamp against a local struct
