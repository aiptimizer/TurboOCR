...
void VLMCropPool::worker_loop() {
    ...
    while (!stop_.load(std::memory_order_relaxed)) {
        ...
        // Block until a transfer needs attention, the timeout expires, or
        // submit() calls curl_multi_wakeup(). No extra fd: the wakeup IS the
        // early-return path, and it works identically on every platform.
        int numfds = 0;
