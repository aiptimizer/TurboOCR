...
bool OrtPaddleDet::load_model(const std::string &model_path,
...
std::vector<Box> OrtPaddleDet::run(const cv::Mat &img) {
  ...
  // Output shape: [1, 1, out_h, out_w] — READ IT, do not assume it equals the
  // requested resize. An execution provider is entitled to hand back a map
  // whose geometry differs from the input canvas (a device that cannot honour
  // a dynamic shape may resolve it to a compiled one), and reinterpreting that
  // buffer as resize_h x resize_w silently decodes a MISALIGNED probability
  // map — detection then loses boxes uniformly on every page while still
  // "working". If the buffer is also smaller than assumed, the old code read
  // past its end.
  int out_h = resize_h, out_w = resize_w;
  if (result.shape.size() >= 2) {
    out_h = static_cast<int>(result.shape[result.shape.size() - 2]);
    out_w = static_cast<int>(result.shape[result.shape.size() - 1]);
  }
  if (out_h <= 0 || out_w <= 0 ||
      static_cast<size_t>(out_h) * static_cast<size_t>(out_w) > result.data.size()) {
    std::cerr << "[OrtPaddleDet] engine returned an unusable output shape ("
              << out_h << "x" << out_w << " for " << result.data.size()
              << " floats) — dropping this page rather than decoding garbage\n";
    return {};
  }
  if (out_h != resize_h || out_w != resize_w) {
    static bool warned = false;
    if (!warned) {
      warned = true;
      std::cerr << "[OrtPaddleDet] engine output " << out_h << "x" << out_w
                << " != requested " << resize_h << "x" << resize_w
                << " — using the ENGINE's geometry (this EP is not honouring "
                   "the dynamic input shape)\n";
    }
  }
  cv::Mat pred_map(out_h, out_w, CV_32F, result.data.data());
  ...
  return extract_boxes_from_bitmap(
      pred_map, bitmap_, h, w, out_h, out_w,
