...
std::string normalize(const std::string& in) {
    // Wrapper-macro spans (operatorname/mathrm/text/mathbf) are rewritten IN
    // PLACE with their inner whitespace stripped — the space-collapse passes
    // below then cannot damage them because they no longer contain spaces.
    // (An earlier design swapped them for a sentinel and restored at the end;
    // the direct rewrite replaced it, and the leftover sentinel-restore line
    // was silently rewriting any legitimate literal run of the sentinel text
    // in a formula.)
