...
// GCC/Clang spell the no-alias qualifier `__restrict__`; MSVC spells it
// `__restrict`. It is a pure optimizer hint either way — the GEMVs below are the
// AR loop's hot path and the aliasing guarantee is what lets the inner loop
// vectorize — so dropping it on an unknown compiler is correct, just slower.
...
inline void mv_io(const float *TURBO_RESTRICT x, const float *TURBO_RESTRICT W,
                  int IN, int OUT, const float *TURBO_RESTRICT b,
                  float *TURBO_RESTRICT out) {
...
inline void mv_oi(const float *TURBO_RESTRICT x, const float *TURBO_RESTRICT W,
                  int OUT, int IN, const float *TURBO_RESTRICT b,
                  float *TURBO_RESTRICT out) {
  for (int k = 0; k < OUT; ++k) {
