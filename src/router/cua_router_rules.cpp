...
RouterDecision route(int layout_idx,
  ...
  switch (d.tier) {
    ...
    case ConfidenceTier::Verify:
      // Table verification fires later via VerifyTablePath::should_invoke;
      // formula geometry spot-check is layered through the salvage path.
      // Other classes stay on their class-default.
      break;
