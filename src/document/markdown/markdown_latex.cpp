...
[[nodiscard]] static std::size_t longest_backtick_run(const std::string &s) {
...
[[nodiscard]] std::string inline_code(const std::string &s) {
  // A `…` span can't contain a backtick run as long as its delimiter; the
  // delimiter must be strictly LONGER than the longest run inside. This used
  // to widen to at most `` — which a two-backtick run in the content escapes.
  const std::string fence(longest_backtick_run(s) + 1, '`');
  return fence + " " + s + " " + fence;
}

[[nodiscard]] std::string fenced_block(const std::string &s,
                                       const std::string &info) {
  // Same rule as inline_code, for fenced blocks: a fixed ``` fence is escaped
  // by any embedded triple-backtick (common in documents ABOUT Markdown, and
  // an injection primitive when the text is attacker-controlled) — everything
  // after the embedded fence would parse as live Markdown. Width is computed
  // from the content, minimum 3.
  const std::string fence(std::max<std::size_t>(3, longest_backtick_run(s) + 1),
                          '`');
  return fence + info + "\n" + s + "\n" + fence;
}

[[nodiscard]] std::string escape_md_text(const std::string &s) {
  // OCR'd / PDF-extracted text is UNTRUSTED: in mode=auto a PDF's text layer
  // reaches Markdown byte-exact, so raw `<img onerror=...>` in a document
  // becomes raw inline HTML in the response — stored XSS in any consumer that
  // renders the Markdown (CommonMark/GFM pass inline HTML through by
  // default). Entity-escape the HTML metacharacters (& FIRST), and neutralize
  // a LEADING block marker so document text cannot mint headings, quotes,
  // list items or fences. Rendered output is unchanged: &amp; renders as &.
  std::string out;
  out.reserve(s.size());
  for (char c : s) {
    switch (c) {
