...
// Normalization defaults to REC's (pixel/255 - 0.5)/0.5 — and every in-tree
// caller uses that default, INCLUDING cls (paddle_cls.cpp; the shared
// cls_config.h measured that ImageNet on the classifier is a regression).
// The optional mean/std after `stream` exist for callers that need a third
// distribution. res channels are RGB, so mean/std are in RGB order.
...
// (mean=0, std=1), BGR CHW. Implementation lives in src/analysis/table/slanext/.
...
// Implementation in src/backends/nvidia/stages/table_kernels.cu.
