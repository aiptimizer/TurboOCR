...
public:
  ...
  // run() split in two, so a caller can do other work between the submission
  // and the wait (NvDetector's IDetector::enqueue/collect). submit_forward()
  // launches preprocess + TRT + threshold and returns without synchronizing;
  // collect_boxes() waits and post-processes. run() is the two back to back.
  //
  // SINGLE SLOT: the device scratch below (d_input_/d_output_/d_bitmap_buf_ and
  // every CCL buffer) is one set per instance, so exactly ONE submission may be
  // outstanding — collect before submitting again, which is the seam's rule for
  // every stage (backend/stages.h).
  void submit_forward(const GpuImage &gpu_img, int orig_h, int orig_w,
                      cudaStream_t stream);
  [[nodiscard]] std::vector<Box> collect_boxes(int orig_h, int orig_w,
                                               cudaStream_t stream);

  // The largest run_batch() below can take in one submission. PUBLIC because it
  // is the batch buffers' sizing constant AND the answer NvDetector::
  // max_batch_size() owes the seam; while it was private the wrapper had to
  // report the default 1, i.e. "no batched path", for a detector that has one.
  ...
  // Returns one vector<Box> per image. imgs.size() must be <= kMaxBatchSize.
...
private:
  // Extents the outstanding submit_forward() used; read by collect_boxes().
  ...
  // (kMaxBatchSize is declared in the public section above — it is part of the
  // batched interface's contract, not just a buffer-sizing detail.)
