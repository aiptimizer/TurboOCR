...
public:
  ...
  // Drop every baked graph, so the next bake_graph() records against whatever
  // buffers the caller now holds. PUBLIC because that is the whole content of
  // IEngine::reset(), whose contract is "drop captured graph/binding before
  // buffers change" — while this was private, TrtEngineAdapter::reset() could
  // only be a no-op, and a no-op there is not a missing feature but a hazard: a
  // baked cudaGraphExec_t holds the device addresses it was recorded with, so
  // replaying one after the caller rebinds its buffers reads freed memory.
  // After this call the engine is un-baked and launch_baked() fails until the
  // caller bakes again, which is exactly what reset() promises.
