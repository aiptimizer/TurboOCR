...
#include "nvidia/engine/engine_internal.h"
...
bool read_trt_fp16() {
  // TRT_FP16=0 builds fp32 engines. An opt-OUT, not a tuning knob: TRT
  // 10.15's fp16 foreign-node compilation fails outright on some older
  // graphs (the legacy det_v5 on sm120 dies with Error Code 10 "Could not
  // find any implementation for node {ForeignNode[x_cast...]}" at every
  // workspace/opt-level/profile, even on an empty GPU) — fp32 is the only
  // way to build them. Costs roughly 2x runtime on the affected engine;
  // leave ON for every shipping model.
  static const int v = env::env_int("TRT_FP16", 1, 0, 1);
  return v == 1;
}
...
std::string get_engine_cache_dir() {
  // User override
  if (std::string dir = env::env_or("TRT_ENGINE_CACHE", ""); !dir.empty())
    return dir;

  // Try ~/.cache/turbo-ocr/
  // HOME is read raw on purpose: it is the process's ambient identity, not a
  // TurboOCR knob, and listing it in the "environment overrides in effect"
  // inventory would be noise in every single startup log.
...
std::string get_cached_engine_path(const std::string &onnx_path,
  ...
  // file_time_type::rep is implementation-defined: int64 on libstdc++, but
  // __int128_t on libc++, where std::to_string then matches no overload and the
  // build fails with "call to 'to_string' is ambiguous". Only libstdc++ ever
  // compiled this file, so the cast is what makes a libc++ NVIDIA build
  // possible at all. Nanoseconds since the epoch stay inside int64 until 2262.
  const auto onnx_mtime = static_cast<long long>(
      fs::last_write_time(onnx_path).time_since_epoch().count());
  ...
  // fp32 engines are different artifacts; give them their own hash space so
  // toggling TRT_FP16 never silently serves the other precision's cache.
  if (!detail::read_trt_fp16()) key += ":fp32";
