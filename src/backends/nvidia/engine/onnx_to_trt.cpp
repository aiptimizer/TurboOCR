...
#include <cstring>
#include <filesystem>
// Process id, used only to make the temp engine filename unique per writer so
// two concurrent builds cannot clobber each other's partial plan. POSIX spells
// it getpid() in <unistd.h>; Windows spells it _getpid() in <process.h> and has
// no <unistd.h> at all. This is the entire POSIX surface of the NVIDIA arm.
#if defined(_WIN32)
#include <process.h>
namespace { inline long turbo_pid() { return static_cast<long>(_getpid()); } }
#else
#include <unistd.h>
namespace { inline long turbo_pid() { return static_cast<long>(::getpid()); } }
#endif
#include <fstream>
#include <iostream>
#include <memory>
...
static bool build_engine(const std::string &onnx_path,
  ...
  if (type == "layout") {
    ...
    size_t cap = 4ULL << 30;
    if (std::string env = env::env_or("TRT_DET_WORKSPACE_GB", ""); !env.empty()) {
      int gb = std::atoi(env.c_str());
      if (gb >= 1 && gb <= 24) {
  ...
  // TRT_FP16=0 skips the flag entirely (fp32 build) for graphs whose fp16
  // compilation fails on this TRT version (engine_internal.h).
  if (!strongly_typed && detail::read_trt_fp16())
    config->setFlag(nvinfer1::BuilderFlag::kFP16);
  else if (!strongly_typed)
    std::cout << "[TRT] TRT_FP16=0 — building fp32 engine\n";
  ...
  const auto tmp_path = trt_path + ".tmp." + std::to_string(turbo_pid());
