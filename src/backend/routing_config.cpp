...
// ppformulanet_s     = fast 2-layer model, in-process C++ FAST host-loop, ~10 pg/s
//                      (the default — fastest). NOTE: the shipped bundle's weights
//                      ARE PP-FormulaNet_plus-S — verified 2026-08-05 by rebuilding
//                      models/formula/ppformulanet_s byte-identically from paddle's
//                      official plus-S download (export_ppformulanet_plus_s_fast.py).
//                      "ppformulanet_s" stays the config token; the accurate name
//                      "ppformulanet_plus_s" is accepted and normalized to it.
...
// auto                = composite: plus-S over all crops, plus-M re-run on the crops
//                      whose plus-S output shows CJK it mangled. EN crops keep plus-S
...
// Alias normalization — the ONE seam every engine string passes through (env,
// config file, inline /infer backend), so backend comparisons keep a single
// token and the alias can never fork behavior per transport.
std::string canonical_engine(std::string e) {
  if (e == "ppformulanet_plus_s") return "ppformulanet_s";
  return e;
}
...
RoutingTable synth_from_env() {
  ...
  std::string fb = canonical_engine(env_or("FORMULA_BACKEND", "ppformulanet_s"));
  if (!engine_valid_for("formula", fb))
    ...
    TOCR_LOG_ERROR("FORMULA_BACKEND is not a recognized formula engine, formula DISABLED",
                   "value", fb,
                   "expected",
                   "ppformulanet_s|ppformulanet_plus_s|ppformulanet_plus_m|auto|vlm");
  {
...
BackendSpec parse_backend(const std::string &name, const json &j) {
  ...
  try {
    ...
    if (b.kind == Kind::Local) {
      b.engine     = canonical_engine(j.value("engine", ""));
...
RoutingTable parse_file(const std::string &path) {
  ...
  // --- validation ---
  // `fallback` is parsed for forward compatibility but FAILOVER IS NOT
  // IMPLEMENTED — nothing reads the field after this point. An operator who
  // configures it is planning on failover that will never happen, which is
  // worse than a load error (the refusal-beats-silent-no-op rule): refuse,
  // naming the truth, until a consumer exists. It must also name a real
  // entry so the message can never mislead about WHY it was rejected.
  for (const auto &[bname, spec] : t.backends) {
    if (!spec.fallback || spec.fallback->empty()) continue;
    if (!t.backends.count(*spec.fallback))
      throw RoutingConfigError("ROUTING_DANGLING_REF: backend '" + bname +
                               "' declares fallback '" + *spec.fallback +
                               "' which is not in backends{}");
    throw RoutingConfigError(
        "ROUTING_FALLBACK_UNIMPLEMENTED: backend '" + bname +
        "' declares fallback '" + *spec.fallback +
        "', but failover is not implemented in this build — the field would "
        "be silently ignored. Remove it (and rely on per-request "
        "route_table/route_formula overrides) until failover ships");
  }
  ...
  for (const char *m : {"text", "table", "formula"}) {
    if (t.routes.try_emplace(m, env.routes[m]).second) {
