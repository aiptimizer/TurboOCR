...
void register_observability_middleware() {
  ...
  drogon::app().registerPostHandlingAdvice(
      [](const drogon::HttpRequestPtr &req,
        ...
        // rejects them.
        // Set by the request gate via attributes (headers are client-writable
        // and would reflect a client-supplied value into the response).
        std::string ignored;
        if (req->getAttributes()->find("turbo.ignored_params"))
          ignored = req->getAttributes()->get<std::string>("turbo.ignored_params");
        if (!ignored.empty()) {
