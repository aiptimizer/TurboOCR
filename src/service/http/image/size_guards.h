...
// Internal to src/service/http/image/: the decompression-bomb size guards shared by
...
[[nodiscard]] inline bool reject_image_size(int w, int h,
  ...
  cb(server::error_response(decode::image_size_error_code(v),
                            decode::image_size_error_message(v, w, h)));
  return true;
}

// Two-stage check for the image routes:
//   1. Pre-decode header sniff (PNG / JPEG / WebP / GIF / TIFF / BMP): refuses
//      oversized inputs without ever calling the decoder, defending against
//      decompression bombs (a 1 KB PNG can claim 100k×100k → 30 GB decode
//      buffer; a ~50 B BMP header can drive the same allocation).
//   2. Post-decode check on the resulting cv::Mat: catches any residual format
//      whose header the sniff could not parse.
