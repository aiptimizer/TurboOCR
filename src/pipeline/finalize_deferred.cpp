...
void finalize_stage(std::vector<PendingCrop> &crops,
  ...
  if (degraded > 0) {
    degraded_flag = true;
    // APPEND, never assign — the rule ocr_pipeline_detail.h's shared writer
    // states and this hand-rolled copy violated: the dispatch path can have
    // already written a warning into the SAME field (async submit
    // under-return), and assigning here replaced that correct diagnosis with
    // this one.
    const std::string w =
        std::string(stage) + " stage degraded: " + std::to_string(degraded) +
        " of " + std::to_string(crops.size()) + " region(s) produced no " +
        artifact +
...
void finalize_deferred(OcrPipelineResult &out) {
  ...
  static const long kFinalizeTimeoutMs = [] {
    long v = env::env_int("FINALIZE_DEFERRED_TIMEOUT_MS", 120000, 1, 24 * 60 * 60 * 1000);
    ...
    const long req = env::env_int("REQUEST_TIMEOUT_MS", 60000, 0, 24 * 60 * 60 * 1000);
