// Lifecycle + model loading + warmup for OcrPipeline. Runtime helpers live in
...
bool OcrPipeline::load_router_models(
  ...
  const backend_routing::RoutingTable formula_routing = backend_routing::load_routing_config();
...
bool OcrPipeline::load_table_backend() {
  ...
  const backend_routing::RoutingTable table_routing = backend_routing::load_routing_config();
