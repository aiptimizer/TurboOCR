...
The tree is split by language first — `tests/cpp/` is compiled, everything
else is Python. `tests/README.md` has the full map; the short form:

```
tests/
  conftest.py                    # Shared fixtures (image generation, server URLs)
  run_all.py                     # Master runner (pytest wrapper + ctest for cpp)
  requirements.txt               # pytest, requests, grpcio, Pillow, etc.
  cpp/                           # Catch2 suites -> build/turbo_ocr_tests, by domain
    backends/                    # standalone cross-backend conformance/bench binaries
  integration/                   # service/ image/ pdf/ structure/ grpc_api/
  regression/                    # Accuracy regression, ordering consistency
  accuracy/                      # Ground-truth F1/CER per fixture
  stress/                        # 60 s soaks (opt-in)
  benchmark/                     # perf/ scoring/ router/ comparison/ (plain scripts)
  e2e/                           # docker + native-server matrix drivers
  fixtures/                      # images, PDFs, expected/*.json
```
...
python tests/run_all.py --suite integration # just the endpoint suite
python tests/run_all.py --suite cpp         # the C++ suite, via ctest
```

Every Python test requires a running server (they are black-box HTTP/gRPC
tests). The C++ suite does not — it links the libraries directly.
...
```bash
python tests/run_all.py --suite integration --suite regression
```
...
curl -X POST http://localhost:8000/ocr/raw \
  --data-binary @tests/fixtures/images/png/business_letter.png \
...
- Best used with the CPU-only build:
  ...
  valgrind --leak-check=full ./build_valgrind/turboocr-server
  ```
...
- The kernels in `src/backends/nvidia/kernels_cuda/*.cu` (batch_roi_warp, argmax, fused_resize_normalize,
...
python tests/benchmark/perf/bench_throughput.py --server-url http://localhost:8000
```
...
valgrind --tool=helgrind \
  ...
  ./build_valgrind/turboocr-server
```
...
- `base/geometry/perspective.h` -- perspective transform matrix computations
  (header-only; reached through the TUs that include it)
...
# Single file
clang-tidy -p build src/image/fast_png_decoder.cpp

# All source files
find src -name '*.cpp' | xargs -P$(nproc) -I{} clang-tidy -p build {}

# With fixes applied (careful -- review first)
clang-tidy -p build --fix src/analysis/recognition/ctc_decode.cpp
```
...
./fuzz_base64 corpus/base64/ tests/fixtures/ -max_len=1048576
```
...
# Single test
pytest tests/integration/image/test_ocr_endpoint.py -v -s
...
The project already has C++ gRPC benchmark tools (`tools/bench/grpc_bench.cpp`,
`tools/bench/grpc_burst.cpp`), but ghz provides richer analysis.
...
cat tests/fixtures/images/png/business_letter.png | base64 > /tmp/test_b64.txt
...
IMAGE_B64 = base64.b64encode(
    Path("tests/fixtures/images/png/business_letter.png").read_bytes()
).decode()
...
class OCRUser(HttpUser):
    ...
    def ocr_raw(self):
        self.client.post(
            "/ocr/raw",
            data=Path("tests/fixtures/images/png/business_letter.png").read_bytes(),
            headers={"Content-Type": "image/png"},
        )
...
The project has comprehensive Python benchmarks, grouped by what they
measure (all under `tests/benchmark/`, all plain scripts — pytest does not
collect them):

- `perf/bench_latency.py` -- single-request latency percentiles
- `perf/bench_throughput.py` -- maximum requests/sec at concurrency 1-32
- `perf/bench_stress.py` -- sustained soak driver
- `perf/bench_matrix.py` -- the 3-phase orchestrator; writes `perf/LATEST.{md,json}`
- `scoring/bench_per_document.py` -- per-fixture F1 + latency (`PER_DOCUMENT.{md,json}`)
- `scoring/bench_funsd_local.py` -- FUNSD bag-of-words F1
- `scoring/bench_ocr_tiers.py` -- tiny/small/medium accuracy vs speed
- `router/bench_cua_router.py` -- CUA-router scenarios + regression verdict
- `comparison/` -- head-to-head against other OCR engines

```bash
python tests/benchmark/perf/bench_matrix.py
...
The `tests/fixtures/` directory contains real test images and PDFs. Use the existing
...
Stage 3: Functional Tests (requires running server)
  - python tests/run_all.py --suite integration --suite regression --suite accuracy
...
Since the project uses Docker (`docker/Dockerfile`, `--target nvidia`), sanitizer builds
...
3. Build normally and run `python tests/run_all.py --suite cpp --suite integration`
