...
- `src/document/markdown/markdown_export.cpp` (+ `markdown_latex.cpp`, `markdown_columns.cpp`,
  `markdown_assets.cpp`)
...
```cpp
#include "turbo_ocr/document/markdown/markdown_export.h"
using namespace turbo_ocr::markdown;
...
This module is **already wired and shipped** as `POST /ocr/markdown` on
EVERY backend: `register_ocr_markdown_route`
(`src/service/http/image/markdown_route.cpp`) is registered unconditionally
by the unified server (`src/service/server/unified/server_main.cpp`). The
sections below document the wiring as built; historical snippets that name
`register_ocr_markdown_route_gpu` / `TURBO_HTTP_GPU_SRCS` describe the
pre-merge GPU-only wiring.
...
That makes it available to whichever `turboocr-server` the configure builds (the
unified one from `src/service/server/unified/unified_server.cmake` or the GPU one — both link
`turbo_ocr_common` transitively) and to the test exe.
...
page pixels are needed to crop figures). The handler has its own TU,
`src/service/http/image/markdown_route.cpp`. Sketched below for shape only — that
file is the source of truth, and the live handler has moved on from this excerpt:
...
It is registered in `register_image_routes(...)`
(`src/service/http/image/raw/raw_route.cpp`) alongside the other
...
The CPU/unified server does **not** yet expose `/ocr/markdown`. The place to add
it is `src/service/http/unified_routes.cpp`, typed on `pipeline::UnifiedPipelinePool`
like the batch route already there: `UnifiedOcrPipeline::run_with_layout` returns
the same `OcrPipelineResult` this module consumes, so no `InferResult` repacking
is needed — assign layout ids, then call `render_markdown_with_assets`.
