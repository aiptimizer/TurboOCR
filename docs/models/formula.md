...
!!! warning "The `fast/` artifacts"
    ...
    `scripts/models/fetch/fetch_release_models.sh` places them at
    `formula/ppformulanet_s/fast/` — nothing has to be generated to run the
    shipped model. To export them for **different** weights, use
    `scripts/models/onnx/export_ppformulanet_plus_s_fast.py` (the committed
    exporter this page's intro references; an earlier revision of this
    section pointed at a pre-release scratch script that was never
    published). The `fast/` graphs are **required** on GPU: if any is
