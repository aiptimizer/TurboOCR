...
!!! abstract "TL;DR"
    Four unary RPCs (`Recognize`, `RecognizeBatch`, `RecognizePDF`,
    `Health`). Default listen address `localhost:50051`. Every
    response carries a `json_response` byte blob — the same JSON the
...
`INVALID_PARAMETER`, `BASE64_DECODE_FAILED`, `IMAGE_DECODE_FAILED`, `GPU_DECODE_FAILED` (`UNAVAILABLE`),
...
!!! tip "Use the JSON fast path"
    ...
    response on a 35-detection page (see `proto/ocr.proto:64-68`). The
    repeated `results` + `num_detections` fields are still populated
    for clients that don't want to parse JSON.

When the server is launched with `--grpc-response-mode=json_only` (see
...
!!! info "See also"
    ...
    - [Build → Docker](../build/docker.md) — `TURBO_OCR_GRPC_PORT` and
